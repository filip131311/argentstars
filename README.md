...
M5Paper e-ink dashboard with swipeable screens:
...
Swipe left/right on the panel to move between screens; they form a cycle. The
screen you leave it on stays until the next swipe and keeps its own refresh
cadence (hourly for stars, once a day for the countdown).

Between refreshes the device deep-sleeps and wakes either from the timer or
from the touch panel's interrupt line. On wake it connects to Wi-Fi only when
the shown screen's refresh is due: the stars screen fetches
...
Because the touch panel must stay powered to detect a swipe, the device no
longer powers off completely on battery — it deep-sleeps with the main power
rail held. Expect days–weeks on a charge rather than months.
...
- Pure date/swipe helpers live in `include/timeutil.h` and have a host test:
